...
import { PersonStanding, Calendar, Star, Trash2 } from 'lucide-react';
import { useEffect, useState } from 'react';
import { toast } from 'sonner';

import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from '@/components/ui/tooltip';
import { Dialog, DialogContent, DialogHeader, DialogTitle } from '@/components/ui/dialog';
import { SimilarAvatarsDialog } from '@/components/SimilarAvatarsDialog';
import { AvatarInfoDialog } from '@/components/AvatarInfoDialog';
...
const PerformanceRating: any = ({ rating, platform }: { rating: any, platform: any }): JSX.Element => {
    const ratingValue: any = rating !== null ? rating : 0;
    const ratingData: any = ratings[ratingValue];
...
export default function AvatarCard({ avatar, fromGraph = false }: { avatar: any, fromGraph?: boolean }): JSX.Element {
    ...
    const formatDate: (dateString: any) => string = (dateString: any): string => {
        return new Date(dateString).toLocaleDateString();
    };

    const fetchSimilarAvatars: any = async (avatarId: string): Promise<any> => {
        try {
            const response: any = await window.electron.PAW.findSimilar(avatarId);

            if (!response.success) return toast.error('Failed to fetch similar avatars.');

            const results: any = response.data.results || [];

            return results.map((avatar: any, index: any): { avatar: any } => ({
                avatar: avatar
            }));
        } catch (error) {
    ...
    const openSimilarGraph: () => void = (): void => {
    ...
    const checkIfFavorite: () => Promise<void> = async (): Promise<void> => {
        const result: any = await window.electron.Database.checkFavorite(avatar.id);

        setIsFavorite(result.length > 0);
    };

    const addToFavorites: (categoryId: number) => Promise<void> = async (categoryId: number): Promise<void> => {
        if (!categoryId) {
        ...
        const favorited: { success: boolean; exists?: boolean; error?: string } = await window.electron.Database.favoriteAvatar(categoryId, avatar.id);
    ...
    const unfavoriteAvatar: () => Promise<void> = async (): Promise<void> => {
        await window.electron.Database.removeFromFavorites(avatar.id);
        setIsFavoriteDialogOpen(false);
        setIsFavorite(false);
    ...
    useEffect((): void => {
        const checkAuthState: () => Promise<void> = async (): Promise<void> => {
            const authStatus: any = await window.electron.VRChat.getAuthStatus();

            setAuthState(authStatus.state);
        };
    ...
    return (
        <Card className="h-full overflow-hidden bg-card">
            ...
            <div className="p-3 space-y-2">
                ...
                <div className="flex flex-col gap-2">
                    <div className="flex gap-2">
                        {isFavorite ? (
                            <Button onClick={(): void => setIsFavoriteDialogOpen(true)} className="h-8" size="icon">
                        ...
                        <Button disabled={authState !== 'authenticated'} onClick={equipAvatar} className="flex-1 h-8 text-xs" size="sm">
                            <PersonStanding className="ml-1 h-3 w-3"/>{authState === 'authenticated' ? 'Select Avatar' : 'Please Log In'}
                        </Button>
                    </div>
                    <Button onClick={(): void => setIsRefreshDialogOpen(true)} variant="secondary" className="w-full h-8 text-xs" size="sm">
                        Request Refresh
                    </Button>
                    <div className="flex gap-2">
                        <Button disabled={authState !== 'authenticated'} onClick={(): void => setIsInfoDialogOpen(true)} variant="secondary" className="w-full h-8 text-xs" size="sm">
                            More Info
                        </Button>
            ...
            <RefreshDialog avatarId={avatar.id} isOpen={isRefreshDialogOpen} onClose={(): void => setIsRefreshDialogOpen(false)}/>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-1 gap-2 max-h-[200px] overflow-y-auto">
                                    {categories.map((category) => (
                                        <div key={category.id} className="p-2 border rounded-md cursor-pointer hover:bg-muted" onClick={(): void => {
            ...
            <AvatarInfoDialog avatarId={avatar.id} isOpen={isInfoDialogOpen} onClose={(): any => setIsInfoDialogOpen(false)} />
            <SimilarAvatarsDialog isOpen={isSimilarGraphOpen} onClose={(): any => setIsSimilarGraphOpen(false)} avatar={avatar} onFetchSimilar={fetchSimilarAvatars}/>
        </Card>
    );
};